...
function bufferFromHex(hexString: string): Buffer {
  return Buffer.from(hexString.slice(2), "hex");
}

// The Merkle tree node contains: (address account)
export type MerkleTreeLeaf = string[];
const LEAF_TYPES = ["address"];
...
export default class MerkleTree {
  ...
  constructor(leaves: MerkleTreeLeaf) {
    ...
    this.height = (n - 1).toString(2).length + 1;
